...
import com.snow.mkremins.fanciful.amoebaman.util.ArrayWrapper;
import com.snow.mkremins.fanciful.amoebaman.util.Reflection;
 ...
 * Modified for more Book related Functions.
 */
public class FancyMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable {
	...
	/**
	 * Sets the text of the current editing component to a value.
	 * Splits text at ChatColor chars and adds a new Segment with that ChatColor
	 * @param text The new text of the current editing component.
	 * @return This builder instance.
	 */
	public FancyMessage colorText(String text) {
		if (!text.contains("§")) {
			return text(text);
		}
		MessagePart latest = latest();

		String colorSplit[] = text.split("§");
		ChatColor color;
		ChatColor style = null;
		boolean mod = false;
		for (String cs : colorSplit) {
			if (cs.length() == 1) {
				color = ChatColor.getByChar(cs.charAt(0));
				if (!color.isColor()) {
					style = color;
					color = null;
				} else {
				...
				mod = true;
				latest.text = TextualComponent.rawText(cs.substring(1));
				if (color != null) {
					latest.color = color;
				}
				if (style != null) {
	...
	/**
	 * Set the behavior of the current editing component to instruct the client to change the page of the currently opened book to the specified page when the currently edited part of the {@code FancyMessage} is clicked.
	 * @param page The page of the book to display
	 * @return This builder instance.
	 */
	public FancyMessage page(final int page) {
	...
	*/
	 ...
	 * @return This builder instance.
	 */
	public FancyMessage then(final FancyMessage text) {
		if (!latest().hasText()) {
			throw new IllegalStateException("previous message part has no text");
		}
		if (text == this) {
			throw new IllegalArgumentException("cannot append itself");
		}
		messageParts.addAll(text.messageParts);
		dirty = true;
		return this;
	}
	...
	public Object createChatBaseComponent() {
		try {
			return createChatBaseComponent(toJSONString());
		} catch (IllegalArgumentException e) {
			Bukkit.getLogger().log(Level.WARNING, "Argument could not be passed.", e);
		} catch (IllegalAccessException e) {
			Bukkit.getLogger().log(Level.WARNING, "Could not access method.", e);
	...
	private Object createChatBaseComponent(String json) throws IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
		...
		return fromJsonMethod.invoke(nmsChatSerializerGsonInstance, json, Reflection.getNMSClass("IChatBaseComponent"));
	}
	...
	private Object createChatPacket(String json) throws IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
		return toPacket(createChatBaseComponent(json));
	}

	private Object toPacket(Object chatBaseComponent) throws IllegalAccessException, InvocationTargetException, InstantiationException {
		return nmsPacketPlayOutChatConstructor.newInstance(chatBaseComponent);
	}
